...
//催眠师催眠
                ...
                if (pc.Is(CustomRoles.Karen) && pva.DidVote && pc.PlayerId != pva.VotedFor && pva.VotedFor < 253 && !pc.Data.IsDead)
                {
                    if (!Karen.hasVoted)
                    {
                        var voteTarget = Utils.GetPlayerById(pva.VotedFor);
     ...
     //       coloredRole = Utils.ColorString(Utils.GetRoleColor(CustomRoles.Sidekick), GetRoleString("Temp.Blank") + coloredRole.RemoveHtmlTags());
        ...
        switch (Options.CEMode.GetInt())
        {
            case 0:
                ...
                break;
            case 1:
                if (player.GetCustomRole().IsImpostor() || player.Is(CustomRoles.Parasite) || player.Is(CustomRoles.Troll) || player.Is(CustomRoles.Crewpostor) || player.Is(CustomRoles.Refugee) || player.Is(CustomRoles.Convict))
                ...
                else if (player.GetCustomRole().IsNeutral() && !player.GetCustomRole().IsCoven() && !player.Is(CustomRoles.Parasite) && !player.Is(CustomRoles.Troll) && !player.Is(CustomRoles.Refugee) && !player.Is(CustomRoles.Crewpostor) && !player.Is(CustomRoles.Convict))
    ...
    public static Dictionary<byte, int> CustomCalculateVotes(this MeetingHud __instance)
    {
        ...
        for (int i = 0; i < __instance.playerStates.Length; i++)
        {
            ...
            if (ps.VotedFor is not 252 and not byte.MaxValue and not 254)//该玩家面板里是否投了该玩家
            {
                ...
                //市长附加票数
                if (CheckForEndVotingPatch.CheckRole(ps.TargetPlayerId, CustomRoles.Mayor) && ps.TargetPlayerId != ps.VotedFor) VoteNum += Options.MayorAdditionalVote.GetInt();
                if (CheckForEndVotingPatch.CheckRole(ps.TargetPlayerId, CustomRoles.Manager) && ps.TargetPlayerId != ps.VotedFor) VoteNum += Options.ManagerVoteCount.GetInt();
                if (CheckForEndVotingPatch.CheckRole(ps.TargetPlayerId, CustomRoles.Knighted) && ps.TargetPlayerId != ps.VotedFor) VoteNum += 1;
                if (CheckForEndVotingPatch.CheckRole(ps.TargetPlayerId, CustomRoles.Glitch) && ps.TargetPlayerId != ps.VotedFor) VoteNum = 0;
                if (CheckForEndVotingPatch.CheckRole(ps.TargetPlayerId, CustomRoles.Vindicator) && ps.TargetPlayerId != ps.VotedFor) VoteNum += Options.VindicatorAdditionalVote.GetInt();
                if (Options.DualVotes.GetBool())
                    if (CheckForEndVotingPatch.CheckRole(ps.TargetPlayerId, CustomRoles.DualPersonality) && ps.TargetPlayerId != ps.VotedFor) VoteNum += VoteNum;
                //窃票者附加票数
